...
const axios = require('axios'); // Untuk melakukan HTTP request ke server Growtopia
const app = express();
...
// Endpoint untuk cache (contoh: /cache/<IP>/)
app.get('/:ip/cache/*', async (req, res) => {
  try {
    const ip = req.params.ip; // Ambil IP dari parameter
    const path = req.params[0]; // Ambil path setelah IP
